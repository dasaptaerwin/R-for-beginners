...
## Mengubah format data (reshape)

Format yang dimaksud di sini adalah mengubah format data lebar (_wide_) data menjadi format data panjang (_long_) atau sebaliknya. Dalam bahasa sehari-hari, _reshape_ sama dengan _transpose_. Dalam R ini dilakukan dengan menggunakan perintah:

- jangan lupa diawali dengan `install.packages("reshape")` dan `library(reshape)`,
- `melt()` dan `cast()`.

Si Pembuat, Hadley Wickham menggunakan prinsip melelehkan ("melt") data frame untuk kemudian dicetak ("cast") dengan bentuk data frame sesuai keinginan kita. Data "dilelehkan" untuk membuat kombinasi id-variabel yang unik untuk setiap barisnya. 
